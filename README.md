...
1) Clean Dataset: Define what "clean dataset" means and figure out ways to execute it (e.g drop missing values or impute them):
    ...
    *: Technically, all passenger's cabins were located in Decks A, B, C, D, E, F, and G. Most of the first class passengers' cabins, were in the decks A, B, and C, which were exclusively allocated for them. The rest of the first class's cabins were situated in decks D and E, yet these decks had also cabins, which were ment for all passengers. The last two decks, namelly decks F and G, had been provided only to second and third class passengers. Given these pieces of information, and the total number of passengers in the dataset, try to solve the problem stated above in paragraph c.
